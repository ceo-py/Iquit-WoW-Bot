...
def compute_time_modifier(par_time_percentage):
    ...
    if percentage_offset > TimerConstants["Threshold"]:
        ...
        return percentage_offset * TimerConstants["MaxModifier"] / TimerConstants["Threshold"]
    elif percentage_offset == 0:
        return 0
    elif percentage_offset > -TimerConstants["Threshold"]:
        return percentage_offset * TimerConstants["MaxModifier"] / TimerConstants["Threshold"] - TimerConstants["DepletionPunishment"]
    else:
        return None
...
def compute_scores(dungeon_id, level, time_in_seconds, par_time):
    ...
    return {
        ...
        "parTimePercentageString": pad_string(format_number(par_time_fraction * 100, 2, True), 7) + "%"
    }
...
def build_key_data_string(blizzard_scores, affix_score_data):
    ...
    computed_key_score = compute_affix_score_sum(computed_tyrannical, computed_fortified)
...
def compute_tt_enhancement(dungeon_id, par_time, blizzard_affix_score_data, blizzard_total_score):
    ...
    if blizzard_affix_score_data is not None:
        for info in blizzard_affix_score_data:
            computed_affix_score_data[info["name"]] = compute_scores(dungeon_id, info["level"], info["durationSec"], par_time)
...
def print_score_table():
    for dungeon_id, abbreviation in DungeonAbbreviations.items():
        ...
        print(compute_tt_enhancement(dungeon_id, par_time, blizzard_affix_score_data, blizzard_total_score))
    current_score = 500  # Placeholder for C_ChallengeMode.GetOverallDungeonScore()
    print("========================================")
    print(f"Total        {pad_string(current_score, 3)} | {pad_string(round(total_score), 3)}")
    print("========================================")
