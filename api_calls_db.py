...
class APICALLDB:
    ...
    def api_call_channels(self, channel_id):
        ...
        return requests.request("POST", self.api_url, headers=self.headers, data=self.payload).json()["documents"]
    ...
    def search_for_character_in_api(self, channel_id, c_region, c_realm, c_name):
        self.payload = json.dumps({
            ...
            "database": "WOW",
            "dataSource": "MainData",
            "filter": {f"Region": c_region, "Realm": c_realm, "Character Name": c_name}
        })
        return requests.request("POST", self.api_url, headers=self.headers, data=self.payload).json()["documents"]
